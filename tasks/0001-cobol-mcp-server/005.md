...
- [ ] All file existence checks pass: COBOL source, run.sh, test script, README all exist at their expected paths
- [ ] The COBOL source contains all four divisions (IDENTIFICATION, ENVIRONMENT, DATA, PROCEDURE)
- [ ] The COBOL source uses free-format (contains the free-format directive)
- [ ] The COBOL source contains ACCEPT and DISPLAY statements for stdin/stdout I/O
- [ ] The COBOL source contains handling strings for all five MCP methods: initialize, notifications/initialized, ping, tools/list, tools/call
- [ ] The COBOL source contains the protocol version `2025-11-25`
- [ ] The COBOL source contains the server name `cobol-mcp-server`
- [ ] The COBOL source contains all three tool names: add, format_currency, validate_date
- [ ] The COBOL source contains error codes -32601 and -32700
- [ ] The COBOL source contains at least one PICTURE clause used for currency formatting
- [ ] `run.sh` is executable and references both `cobc` and the COBOL source file
- [ ] `test/test-mcp-server.sh` is executable
- [ ] The README mentions all three tools, includes client configuration guidance, and includes GnuCOBOL installation instructions
- [ ] `.gitignore` covers compiled COBOL binaries and build artifacts
- [ ] No obvious structural issues remain — the project is as complete and correct as possible without a compiler
