...
- [x] The `tools/list` response contains exactly three tool objects in the `tools` array
- [x] Each tool in the list has `name`, `description`, and `inputSchema` fields
- [x] The tool names are `add`, `format_currency`, and `validate_date`
- [x] Each tool's `inputSchema` describes its expected arguments with types and required fields
- [x] Calling `add` with two numeric values returns a result containing their sum
- [x] Calling `add` with non-numeric values returns a result with `isError` true
- [x] Calling `format_currency` with a numeric value returns a dollar-formatted string with dollar sign, commas, and decimal point
- [x] Calling `format_currency` with a large number (e.g., millions) produces correct comma separation
- [x] Calling `format_currency` with zero returns a sensible formatted result (not blank or garbled)
- [x] Calling `validate_date` with a valid date returns a result confirming validity
- [x] Calling `validate_date` with an invalid month returns a result indicating the month is invalid
- [x] Calling `validate_date` with an invalid day for the month returns a result indicating the day is invalid
- [x] Calling `validate_date` with February 29 on a leap year (e.g., 20240229) returns valid
- [x] Calling `validate_date` with February 29 on a non-leap year (e.g., 20250229) returns invalid
- [x] Calling `validate_date` with a non-8-character or non-numeric string returns an error result
- [x] Calling `tools/call` with an unknown tool name returns a result with `isError` true (not a JSON-RPC protocol error)
- [x] All tool results use the text content type with a `type` of `text` and a `text` field
- [x] The COBOL source uses a PICTURE clause for currency formatting (this is the whole joke — verify the PIC clause exists in the source)
