...
- [ ] The dispatch point in the main loop routes to distinct handler paragraphs/sections based on the extracted method name
- [ ] Sending an `initialize` message produces a response containing `protocolVersion` with value `2025-11-25`
- [ ] The initialize response contains `serverInfo` with `name` set to `cobol-mcp-server`
- [ ] The initialize response contains a `capabilities` object that includes `tools`
- [ ] The initialize response echoes back the request `id` from the incoming message
- [ ] Sending a `notifications/initialized` message produces no output on stdout
- [ ] Sending a `ping` message produces a response with an empty result object and the matching request `id`
- [ ] Sending a message with an unknown method produces a JSON-RPC error response with code -32601
- [ ] A malformed message that cannot be parsed produces a JSON-RPC error response with code -32700
- [ ] A routing path exists for `tools/list` (even if it returns a placeholder response for now)
- [ ] A routing path exists for `tools/call` (even if it returns a placeholder response for now)
- [ ] The `tools/call` routing extracts the tool name from `params.name` before dispatching
- [ ] All responses include `"jsonrpc":"2.0"` (or equivalent with spacing)
- [ ] The server continues running and accepting input after handling any of these messages (does not exit after one request)
