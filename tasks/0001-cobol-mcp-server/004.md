...
- [ ] `run.sh` exists at the project root and is executable
- [ ] `run.sh` checks for `cobc` availability and prints a helpful error if not found
- [ ] `run.sh` compiles the COBOL source file using `cobc` and runs the resulting binary
- [ ] `run.sh` passes stdin/stdout through to the compiled server for stdio transport
- [ ] `test/test-mcp-server.sh` exists and is executable
- [ ] The test script checks for GnuCOBOL and skips gracefully if not installed
- [ ] The test script includes test cases for initialize, ping, tools/list, tools/call for all three tools, and error handling
- [ ] The test script reports pass/fail results for each test case
- [ ] The test script exits with non-zero status if any test fails
- [ ] `README.md` exists and is non-empty
- [ ] The README explains what the project is and why it exists
- [ ] The README documents all three tools by name with their arguments
- [ ] The README includes GnuCOBOL installation instructions
- [ ] The README includes Claude Desktop configuration (mentions `claude_desktop_config.json` or equivalent)
- [ ] The README includes instructions for running the server and the test script
